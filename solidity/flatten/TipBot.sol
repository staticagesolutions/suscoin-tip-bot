...
// Root file: contracts/TipBot.sol
...
contract TipBot is AccessControlEnumerable, ReentrancyGuard {
  ...
  event Tip ( address indexed from,  address indexed tokenAddress, uint256 amount );
  event AirDrop ( address[] indexed accounts);

  // Global variables used in contract
  uint256 public feeRate = 1000000;
  ...
  function airDrop ( 
    ...
    for(uint8 i = 0; i < accountAddress.length; i++){
      (bool success, ) = accountAddress[i].call{value: msg.value }("");
      require(success, "Failed to send Token");
  ...
  function grantAdminRole(
    ...
    // extract memory and decode
    address extractedAddress = abi.decode(data, (address));
    require( extractedAddress == accountAddress, "Address is not valid ");
    grantRole(DEFAULT_ADMIN_ROLE, accountAddress);
  }
  ...
  function revokeAdminRole(
    ...
    address extractedAddress = abi.decode(data, (address));
    require( extractedAddress == accountAddress, "Address is not valid ");
    revokeRole(DEFAULT_ADMIN_ROLE, accountAddress);
  }
}
